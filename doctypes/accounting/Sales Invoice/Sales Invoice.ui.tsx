...
const calculate = (frm: FormType) => {
    const items = frm.get_value("sales_invoice_items") || [];
...
export default function SalesInvoiceScripts() {
    useEffect(() => {
        // Sales Invoice Calculations - Calculate total from invoice items
        zui.form.on("zerp__Sales Invoice", {
            sales_invoice_items: function(frm) {
                calculate(frm);
            },
        ...
        // Sales Invoice List Scripts - Add badge for payment_status field
        zui.list.on("zerp__Sales Invoice", {
